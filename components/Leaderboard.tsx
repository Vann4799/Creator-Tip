...
interface SupporterRow { from_address: string; total: number; count: number; }
...
export function Leaderboard({ username }: { username: string }) {
  ...
  useEffect(() => {
    supabase.from('tips').select('from_address, amount_eth').eq('to_username', username)
      .then(({ data }) => {
        if (!data) return setLoading(false);
        const map: Record<string, SupporterRow> = {};
        data.forEach((t: { from_address: string; amount_eth: number }) => {
          const a = t.from_address;
          if (!map[a]) map[a] = { from_address: a, total: 0, count: 0 };
          map[a].total += Number(t.amount_eth);
          map[a].count += 1;
        });
  ...
  return (
    <div className="space-y-2">
      ...
      {supporters.map((s, i) => (
        <div key={s.from_address} style={CARD}
          ...
          <div className="font-bold text-sm shrink-0">{s.total.toFixed(4)} ETH</div>
        </div>
      ))}
    </div>
  );
